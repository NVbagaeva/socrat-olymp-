/* scripts/lib/sheet-render.mjs — печать листа в PDF.
   ...
   Playwright в зависимостях проекта не объявлен: реестр npm
   в рабочей среде закрыт политикой, обновить lock-файл оттуда
   нельзя. Пакет ищется сначала среди зависимостей проекта, потом
   среди глобальных. Нет ни там, ни там — скрипт говорит, что
   поставить, и выходит с ошибкой, а не падает стеком.
*/
...
async function loadPlaywright() {
  ...
  throw new Error(
    'Playwright не найден. Поставьте его в проект одной командой:\n' +
    '    pnpm add -D playwright\n' +
    'Браузер при этом уже есть, если задан PLAYWRIGHT_BROWSERS_PATH.'
  );
}
