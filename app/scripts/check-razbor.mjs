...
/* scripts/check-razbor.mjs — разборы заданий №4 и №5 набираются.
   ...
   На карточке шаг разбора показывается словами и формулой: поле
   `formula` шага в TeX набирает KaTeX (см. lib/veroyatnost/nabor.ts).
...
try {
  ...
  function proverit(imya, shagi, pryamaya) {
    try {
      ...
      formul += razbor.shagi.filter((shag) => shag.html !== undefined).length;
    } catch (error) {
