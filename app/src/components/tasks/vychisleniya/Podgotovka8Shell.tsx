  ...
  return (
    <section className="prep">
      <header className="prep__head">
        ...
        <Podgotovka8Counter totals={PREP_BLOCKS.map((block) => ({ id: block.id, total: block.zadachi.length }))} />
      </header>
      <PrepChips
        allLabel={prep8Page.allLabel}
        listHref={listHref}
        active={active}
        items={PREP_BLOCKS.map((block) => ({ id: block.slug, title: block.nazvanie, href: `${listHref}${block.slug}/` }))}
      />
      <TabScrollOnMount />
      {children}
    </section>
