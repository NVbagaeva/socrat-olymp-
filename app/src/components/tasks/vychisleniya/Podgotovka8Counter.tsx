...
export function Podgotovka8Counter({ totals }: Podgotovka8CounterProps) {
  ...
  return (
    <>
      ...
      <ProgressBar className="prep__meter" value={total === 0 ? 0 : (solved / total) * 100} label={`Подготовительные задачи: решено ${solved} из ${total}`} />
    </>
  );
}

/** Полоса и строка «N из 8» на карточке блока. */
export function Podgotovka8Meter({ id, total, title }: { id: string; total: number; title: string }) {
  const progress = usePrep8Progress();
  const solved = prep8Solved(progress, id, total);
  return (
    <span className="prep-card__meter">
      <ProgressBar value={total === 0 ? 0 : (solved / total) * 100} label={`${title}: решено задач`} />
