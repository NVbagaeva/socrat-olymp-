...
/* DEMO: пройденные разделы кабинета — та пара, что стоит в шапке темы.
   Это счётчик по разделам, а не по пунктам теории внутри одной подтемы:
   числа разные, и путать их нельзя. Настоящее значение придёт из
   прогресса ученика, когда он появится; до тех пор пара живёт здесь и
   приходит на страницу через lib/storage.ts. */
export interface DemoStudied {
  studied: number;
  total: number;
}

export const demoStudied: DemoStudied = { studied: 6, total: 13 };
