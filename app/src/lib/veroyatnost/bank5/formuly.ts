/**
 * Задание №5, прототипы без рисунка: 5.1 сложение несовместных,
 * 5.2 противоположное событие, 5.4б совместные события (кофейные
 * автоматы), 5.10 испытания до первого успеха. Рисунка у них нет —
 * только шаги с формулами.
 *
...
/* ── 5.1. Сложение несовместных событий ──────────────────────────── */
...
const P01: Prototype = prototip({
  ...
  tip: 'Сложение несовместных событий',
  zadachnik: [1, 4],
  format: 'десятичная',
  okruglenie: tochno,
  uslovie: (p) =>
...
const P04: Prototype = prototip({
  ...
  shagi: (p) => {
    ...
    return [
      {
        text: 'Событие «кофе закончится хотя бы в одном автомате» — объединение совместных событий A и B; их пересечение вычитаем, чтобы не посчитать дважды:',
        formula: `P(A \\cup B) = P(A) + P(B) - P(A \\cap B) = ${tex(q)} + ${tex(q)} - ${tex(q12)} = ${tex(hotyaBy)}`,
        value: hotyaBy,
      },
      {
...
/* ── 5.10. Испытания до первого успеха: патроны ──────────────────── */
...
const P12: Prototype = prototip({
  ...
  tip: 'Испытания до первого успеха',
  zadachnik: [55, 58],
  format: 'целое',
  okruglenie: tochno,
  uslovie: (p) =>
