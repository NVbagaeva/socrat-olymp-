...
/* Здесь условие отсекает исходы двух костей, и удобное число не нужно:
   исходов 36. Для задач блока с процентами («мужчина/женщина,
   пенсионер/не пенсионер») разбор по методическому соглашению
   начинается с условного круглого числа людей — например 10 000, —
   чтобы все категории были целыми; см.
   content-source/veroyatnost-4-5/README.md. Условие задачи при этом
   не меняется. */
const P08: Prototype = {
  id: 'p5-08',
  blok: 'uslovnaya',
