...
export const P03_24: Prototype = {
  ...
  uslovie: (p) =>
    `Площадь боковой поверхности треугольной призмы равна ${ru(num(p, 'S'))}. Через среднюю линию ` +
  ...
  shagi: (p) => {
    ...
    return [
      {
        text: 'Отсечённая часть — призма с основанием, подобным исходному с коэффициентом одна вторая: две её боковые грани — половины исходных, третья (по сечению) равна половине третьей исходной.',
      },
      {
...
export const P03_25: Prototype = {
  ...
  uslovie: (p) =>
    ...
    `ребру. Площадь боковой поверхности отсечённой треугольной призмы равна ${ru(num(p, 's'))}. ` +
...
export const P03_26: Prototype = {
  ...
  uslovie: (p) =>
    `Через среднюю линию основания треугольной призмы, объём которой равен ${ru(num(p, 'V'))}, ` +
  ...
  shagi: (p) => {
    ...
    return [
      {
        text: 'Отсечённая часть подобна исходной призме с коэффициентом одна вторая по основанию: её объём — четверть полного, для любого треугольника в основании.',
      },
      {
...
export const P03_27: Prototype = {
  ...
  uslovie: (p) =>
    'Через среднюю линию основания треугольной призмы проведена плоскость, параллельная боковому ' +
    `ребру. Найдите объём этой призмы, если объём отсечённой треугольной призмы равен ${ru(num(p, 'v'))}.`,
