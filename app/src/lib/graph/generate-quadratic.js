...
function sceneFor(built, task, set) {
  ...
  /* Точка пересечения принадлежит обеим кривым, поэтому цвета ни
     ...
     Координат у неё на чертеже не пишем — как в экзаменационных
     чертежах: очевидная точка пересечения по правилам подтемы стоит
     в узле сетки, и ученик снимает её координаты сам. Подписи есть
     только у кривых. В разборе координаты названы словами, так что
     без подписи на чертеже он остаётся понятным. */
  built.points.filter(function (point) { return point.role === 'cross'; }).forEach(function (point) {
    points.push({ x: point.x, y: point.y, style: 'solid', color: 'cross', label: null });
  });
